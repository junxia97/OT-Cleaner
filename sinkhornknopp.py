...
def cpu_sk(self):
    ...
    for batch_idx, (data, _,_1,_selected) in enumerate(self.pseudo_loader):
    ...
    print("Aggregation of outputs takes {0:.2f} min".format((time.time() - now) / 60.), flush=True)

    # 2. label refurbishment via sinkhorn-knopp:
    if self.hc == 1:
...
def gpu_sk(self):
    ...
    if self.hc == 1:
        ...
        self.model.headcount = self.hc
    print("Aggregation of outputs takes {0:.2f} min".format((time.time() - start_t) / 60.), flush=True)
    # 2. label refurbishment via sinkhorn-knopp:
    if self.hc == 1:
...
def optimize_L_sk_multi(self, nh=0):
    """ label refurbishment via Sinkhorn-Knopp.
    """
    N = max(self.L.size())
    tt = time.time()
